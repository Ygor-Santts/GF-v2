...
router.get("/dashboard", async (req, res) => {
    try {
        ...
        if (year && month) {
            ...
            const endDateForBalance = targetYear === now.getFullYear() && targetMonth === now.getMonth() + 1
                ? today
                : endOfSelectedMonth;
            dateFilter = {
                year: targetYear,
        ...
        const currentTransactions = await Transaction_1.default.find(dateFilter).lean();
        ...
        if (Object.keys(previousPeriodFilter).length > 0) {
            const previousTransactions = await Transaction_1.default.find(previousPeriodFilter).lean();
            previousSummaryData = [
                {
                    _id: "INCOME",
                ...
                },
                {
                    ...
                    total: previousTransactions
                        .filter((t) => t.type === "EXPENSE")
                        .reduce((sum, t) => sum + (t.amount || t.plannedAmount || 0), 0),
                    count: previousTransactions.filter((t) => t.type === "EXPENSE").length,
                },
            ];
        }
        const summary = {
        ...
        const previousIncome = previousSummaryData.find((item) => item._id === "INCOME")?.total || 0;
        const previousExpenses = previousSummaryData.find((item) => item._id === "EXPENSE")?.total || 0;
        // Calcular crescimento percentual
        summary.incomeGrowth = previousIncome > 0
            ? ((summary.totalIncome - previousIncome) / previousIncome) * 100
            : summary.totalIncome > 0 ? 100 : 0;
        summary.expenseGrowth = previousExpenses > 0
        ...
        const monthlyData = [];
        const baseDate = year && month ? new Date(Number(year), Number(month) - 1, 1) : now;
        for (let i = 5; i >= 0; i--) {
            ...
            const monthTransactions = await Transaction_1.default.find(monthFilter).lean();
        ...
        const recentTransactions = await Transaction_1.default.find(dateFilter)
...
router.get("/monthly", async (req, res) => {
    try {
        ...
        const monthlyReports = await Transaction_1.default.aggregate([
            {
                $match: {
...
router.get("/categories", async (req, res) => {
    try {
        ...
        const categoryReports = await Transaction_1.default.aggregate([
            { $match: dateFilter },
            {
                $group: {
...
router.get("/insights", async (req, res) => {
    try {
        ...
        // Dados do mês atual
        const currentData = await Transaction_1.default.aggregate([
            { $match: currentMonth },
            {
                $group: {
                    _id: "$type",
        ...
        const lastData = await Transaction_1.default.aggregate([
            { $match: lastMonth },
            {
                $group: {
                    _id: "$type",
        ...
        if (balance > 0) {
            insights.push(`Você teve um saldo positivo de R$ ${balance.toFixed(2)} este mês!`);
        }
        else {
            alerts.push(`Atenção: Você gastou R$ ${Math.abs(balance).toFixed(2)} a mais do que ganhou este mês.`);
        }
        if (savingsRate >= 20) {
            insights.push(`Excelente! Você conseguiu economizar ${savingsRate.toFixed(1)}% da sua renda.`);
        }
        else if (savingsRate >= 10) {
            insights.push(`Bom trabalho! Você economizou ${savingsRate.toFixed(1)}% da sua renda.`);
            recommendations.push("Tente aumentar sua taxa de economia para 10% ou mais.");
        }
        else {
            recommendations.push("Considere revisar seus gastos para aumentar sua capacidade de poupança.");
        }
        if (currentExpenses > lastExpenses * 1.1) {
            alerts.push("Seus gastos aumentaram significativamente em relação ao mês anterior.");
        }
        res.json({
            insights,
            recommendations,
...
router.get("/monthly-old", async (req, res) => {
    ...
    await (0, ensureMonth_1.ensureMonth)(year, month);
    const data = await (0, report_1.monthlyReport)(year, month);
    res.json(data);
});
