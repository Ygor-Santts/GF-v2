...
const recurringSchema = zod_1.z.object({
    ...
    dayOfMonth: zod_1.z.number().min(1).max(31),
    startDate: zod_1.z.string().optional(),
    endDate: zod_1.z.string().nullable().optional(),
    isActive: zod_1.z.boolean().optional(),
    // Campos de compatibilidade
    name: zod_1.z.string().optional(),
    installments: zod_1.z.number().int().positive().nullable().optional(),
    account: zod_1.z.string().optional(),
});
// GET /api/recurring - Listar transações recorrentes
router.get("/", async (_req, res) => {
    try {
        const list = await Recurring_1.default.find().sort({ createdAt: -1 }).lean();
...
router.get("/:id", async (req, res) => {
    try {
        const { id } = req.params;
        const recurring = await Recurring_1.default.findById(id).lean();
        if (!recurring) {
            return res
                .status(404)
...
router.post("/", async (req, res) => {
    try {
        ...
        const doc = await Recurring_1.default.create(payload);
        await (0, ensureMonth_1.seedRecurringForward)(doc); // auto-gera meses à frente
...
router.put("/:id", async (req, res) => {
    try {
        ...
        const updated = await Recurring_1.default.findByIdAndUpdate(id, patch, { new: true });
        if (!updated) {
            return res
                .status(404)
                .json({ error: "Transação recorrente não encontrada" });
        ...
        if (updated)
            await (0, ensureMonth_1.seedRecurringForward)(updated); // re-semente
        // Retornar no formato esperado pelo frontend
        const response = {
            _id: updated._id,
            type: updated.type,
...
router.patch("/:id/toggle", async (req, res) => {
    try {
        ...
        const updated = await Recurring_1.default.findByIdAndUpdate(id, { isActive }, { new: true });
        if (!updated) {
            return res
                .status(404)
                .json({ error: "Transação recorrente não encontrada" });
...
router.post("/process", async (req, res) => {
    try {
        ...
        const activeRecurring = await Recurring_1.default.find({ isActive: true }).lean();
        let processed = 0;
        let created = 0;
        for (const recurring of activeRecurring) {
            ...
            const existingTransaction = await Transaction_1.default.findOne({
                ...
                amount: recurring.amount,
                description: {
                    $regex: new RegExp(recurring.name || recurring.description || "", "i"),
                },
            });
            if (!existingTransaction) {
                // Criar nova transação
                const transactionDate = new Date(currentYear, currentMonth - 1, recurring.dayOfMonth);
...
router.delete("/:id", async (req, res) => {
    try {
        const { id } = req.params;
        const deleted = await Recurring_1.default.findByIdAndDelete(id);
        if (!deleted) {
            return res
                .status(404)
                .json({ error: "Transação recorrente não encontrada" });
